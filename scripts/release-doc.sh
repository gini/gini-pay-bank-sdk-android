#!/usr/bin/env bash
#
# Commits the documentation to gini-pay-bank-sdk-android's gh-pages branch.
#
...
cp -a ginipaybank/src/doc/build/html ginipaybank/src/doc/build/singlehtml gh-pages/
cd gh-pages
touch .nojekyll
git add -u
git add .
git diff --quiet --exit-code --cached || git commit -a -m 'Gini Pay Bank SDK Integration Guide'
git push